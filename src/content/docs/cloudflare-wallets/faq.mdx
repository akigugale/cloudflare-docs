...
Handles are allocated first come, first served, and a reservation is permanent. Cloudflare does not change a handle after it is reserved, including after a rebrand or a change of name.

## Releasing a handle

Cloudflare does not release a reserved handle back to the pool, and a handle you hold cannot be made available to another account.
...
Cloudflare does not transfer a handle between accounts, and does not combine handles held by several accounts onto one account. A handle stays with the Cloudflare account that reserved it.
